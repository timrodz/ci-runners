defmodule CiRunners.Github.WorkflowJob do
  ...
  def changeset(workflow_job, attrs) do
    workflow_job
    |> cast(attrs, [:github_id, :name, :status, :conclusion, :runner_name, :runner_group_name, :started_at, :completed_at, :workflow_run_id])
    |> validate_required([:github_id, :name, :status, :runner_name, :runner_group_name, :started_at])
